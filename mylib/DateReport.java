...
class DateReport extends JFrame implements ActionListener
{
 ...
 /*public void TotalAmount()
 {
	try
	{
		//String query="Select Sum(Amount) from Bill Where BorrowDate >='2013-01-01' and BorrowDate <='2016-01-01'";
		
		String query="Select Sum(Amount) from Bill Where BorrowDate >='"+fd+"' and BorrowDate <='"+td+"'";
		prstm=cn.prepareStatement(query);
		rsSUM.first();
		rsSUM=prstm.executeQuery();
		System.out.println(rsSUM.getString(1));
		prstm.close();
	}
	catch(Exception ex)
	{
