...
public class ReturnBooks extends JInternalFrame implements ActionListener
{
    ...
    private JButton missButton = new JButton("Missing Book");
    private JButton billButton = new JButton("Make Bill");
    ...
    public ReturnBooks()
	{
        ...
        informationPanel.add(txtFinePerDay);
			txtFinePerDay.setText("2");
			//txtFinePerDay.setEditable(false);
        ...
        returnButtonPanel.add(chkButton);
        returnButtonPanel.add(missButton);
		...
		try
		{
			if (ae.getSource() == chkButton)
			{
				...
				try 
				{
                    ...
                    try
					{
                        String sql = "SELECT * from Transaction where MemberID=" + memid + " and BookID=" + bookid;
                        ...
                        if (rs.next())
						{
                            ...
                            if (today.after(da))
							{
								...
								if(today.getTime() > da.getTime())
								{
									
									chkButton.setVisible(false);
									
									String bid = informationTextField[0].getText();
									String mid = informationTextField[1].getText();
									String fine = txtFinePerDay.getText();
									String amnt = txtTotalFineAmt.getText();
									 
									new BillLate(bid,mid,fine,amnt);
									//dispose();
									...
									new BillLate(data[1],data[2],data[3],data[4],data[5],data[6]);*/
								}
								else
								{
				...
				{
                    JOptionPane.showMessageDialog(null, "Error, Please Complete information");
                }
			...
			if (ae.getSource() == returnButton)
			{
				...
				if (isCorrect())
				{
					...
					if (numberOfBooks >= 0)
					{
						...
						String sql2="DELETE FROM Borrow WHERE BookID ="+data[0]+" AND MemberID ="+data[1];
			...
			if (ae.getSource() == missButton)
			{
				for (int i = 0; i < informationLabel.length; i++)
				{
					if(informationTextField[i].getText().length()==0)
					{
						JOptionPane.showMessageDialog(null, "Please, complete the information", "Warning", JOptionPane.WARNING_MESSAGE);
						break;
					}
