...
public class DemoBorrowBooks extends JFrame implements ActionListener
{
		...
		try
		{
			if(ae.getSource()==borrowButton)
			{
				...
				if (isCorrect())
				{
					...
					rsTrans=stm.executeQuery("SELECT * FROM Transaction order by SrNo");
					...
					if(/*srNumber>=0 || */numberOfBooks >= 1)
					{
						/*String sqlTrans ="INSERT INTO Transaction (SrNo, BookID, MemberID, BorrowDate, ReturnDate) VALUES ('"+srNumber+"','"+ data[0] + "','" + data[1] + "','" + data[2] + "','" + data[3] + "')";
