...
public class SearchBooksAndMembers extends JFrame implements ActionListener
{
			...
			try
			{
				if(ae.getSource()==searchBooksButton)
				{
					if (isBooksDataCorrect())
					{
						...
						try
						{
							String bookQuery = "SELECT BookID, Subject, Language, Title, Version, Author, Publisher, Copyright, Edition, Pages,	NumberOfBooks FROM Books WHERE " + booksData[0] + " LIKE " + booksData[1];
							
							//text field input
							int inputTextField = Integer.parseInt(booksKeyTextField.getText());
							String input = booksKeyTextField.getText();
							rsBook.first();
							//from database
							//"BookID", "Subject", "Title", "Language", "Author", "Publisher"
							int BookID = rsBook.getInt(1);
							String Subject = rsBook.getString(2);
							String Title = rsBook.getString(4);
							String Language = rsBook.getString(3);
							String Author = rsBook.getString(6);
							String Publisher = rsBook.getString(7);
							if (BookID == inputTextField || Subject.equals(input) || Title.equals(input) || Language.equals(input) ||  Author.equals(input)  ||Publisher.equals(input))
							{//new ListSearchBooks(bookQuery);
