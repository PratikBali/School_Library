...
public class RemoveMembers extends JInternalFrame implements ActionListener
{
	...
	private int id;
	//for creating an Internal Panel in the center panel
	private JPanel RemoveMembersberPanel = new JPanel();
	...
	public RemoveMembers()
	{
		...
		//for setting the layout
		RemoveMembersberPanel.setLayout(new FlowLayout(FlowLayout.RIGHT));
		//for adding the button
		RemoveMembersberPanel.add(removeButton);
		//for addint the internal panel to the center panel
		centerPanel.add("South", RemoveMembersberPanel);
				...
				if (isCorrect())
				{
					...
					try
					{
						...
						else
						{
							...
							if(JOptionPane.showConfirmDialog(null,"Do you really wnt to Delete Member","Deleting Member",JOptionPane.OK_CANCEL_OPTION)==0)
